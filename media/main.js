/* global acquireVsCodeApi, RelationsGraph */
(() => {
  'use strict';
  const api = acquireVsCodeApi(), G = RelationsGraph, NS = 'http://www.w3.org/2000/svg';
  const $ = id => document.getElementById(id);
  const editor = window.RelationsEditor(api);
  const svg = $('canvas'), viewport = $('viewport');
  // Clear v1.0's webview persistence; application data now lives exclusively in the sidecar file.
  api.setState(null);
  ...
  function save() {
    ...
    api.postMessage({ type: 'viewState', state });
  }
  function applyUi() {
  ...
  function exportSvg() {
    ...
    api.postMessage({ type: 'export', svg: new XMLSerializer().serializeToString(root) });
  }
  $('open-source').addEventListener('click', () => api.postMessage({ type: 'openSource' }));
  ...
  $('open-view').addEventListener('click', () => { save(); api.postMessage({ type: 'openViewFile' }); });
  $('save-view').addEventListener('click', () => { save(); api.postMessage({ type: 'saveView' }); });
  $('reload-view').addEventListener('click', () => api.postMessage({ type: 'reloadView' }));
  ...
  describeSelection(); api.postMessage({ type: 'ready' });
})();
