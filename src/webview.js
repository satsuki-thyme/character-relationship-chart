...
function getHtml(webview, extensionUri, Uri) {
  ...
  const graph = escapeAttribute(webview.asWebviewUri(Uri.joinPath(extensionUri, 'packages', 'core', 'graph.js')).toString());
...
<body>
${uiMarkup}
<script nonce="${nonce}" src="${graph}"></script><script nonce="${nonce}" src="${resource('editor.js')}"></script><script nonce="${nonce}" src="${resource('main.js')}"></script>
