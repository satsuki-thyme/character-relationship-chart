...
- 人物をドラッグして移動。ドラッグでは詳細欄を開かず、表示中の詳細・選択も維持します。空白をドラッグして図を移動。ホイールで拡大縮小し、**全体**で図を収めます。
- 人物や線・ラベルをクリックすると説明を表示します。**詳細**で詳細欄、**図だけ**で操作欄を開閉できます。
...
- 上限は人物200、関係1000、グループ50。ラベルはすべての関係線より前面に表示し、SVG保存でも同じ順序を保ちます。複雑な図ではラベル同士や人物枠と重なる場合があります。ドラッグや固定座標で調整してください。
...
解析・検証・JSONC編集・表示データ・描画計算は `packages/core/` に集約し、VS Code連携は `src/extension.js` と `src/storage.js` に分かれています。UIは共通ホストの操作を通じて保存・読込を依頼し、VS Code固有の通信・起動・テーマ連携は `media/vscode-*` が担当します。現時点の配布単位は拡張全体です。構成とWeb版への転用状況は [開発ガイド](docs/DEVELOPMENT.md) と [設計](docs/architecture.md) にあります。
